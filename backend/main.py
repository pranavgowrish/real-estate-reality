...
# NOMINATIM_DELAY = 1.0
# EMAIL = "test123@gmail.com" # ENTER YOUR EMAIL HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def convert_address_to_location(address: str):
    ...
    if not data:
        ...
        if zip_code:
            ...
            try:
                ...
                pass
        print(f"[DEBUG] No full address match. Using ZIP: {zip_code} -> Lat: {lat}, Lon: {lon}")
        return zip_code, lat, lon
    ...
    lon = float(result["lon"])
    zip_code = result.get("address", {}).get("postcode")
    print(f"[DEBUG] Geocoded Address: {address} -> ZIP: {zip_code}, Lat: {lat}, Lon: {lon}")
    return zip_code, lat, lon
...
def get_fire_score(lat, lon, csv_path='ca_fire_hazard.csv'):
    ...
    for _, row in df.iterrows():
        ...
        if dist_miles < min_dist_miles:
            ...
            nearest_class = row['HAZ_CLASS']
    if min_dist_miles > MAX_DIST_MILES:
        print(f"[DEBUG] Fire hazard too far (>5mi). Default score 30")
        return 30
    score = {3: 0, 2: 10, 1: 20}.get(nearest_class, 30)
    print(f"[DEBUG] Fire hazard nearest class: {nearest_class}, distance: {min_dist_miles:.2f}mi, score: {score}")
    return score
...
def get_aqi_score(lat, lon, csv_path='ca_city_avg_aqi.csv'):
    ...
    aqi_value = nearest_data['avg_aqi']
    score = 0
    if aqi_value <= 50: score = 30
    elif aqi_value <= 100: score = 25
    elif aqi_value <= 150: score = 20
    elif aqi_value <= 200: score = 5
    print(f"[DEBUG] AQI nearest city distance: {min_dist_km*0.621371:.2f}mi, AQI: {aqi_value}, score: {score}")
    return score
...
def get_emergency_score(services: list, ogLat: float, ogLon: float) -> float:
    ...
    for service in services:
        ...
        if t == "hospital":
            ...
            score += 8 if distance <= 10 else 4 if distance <= 18 else 1
    print(f"[DEBUG] Emergency services score: {score}")
    return score
...
def shop_score(shops: list, lat: float, lon: float) -> float:
    ...
    for s in shops:
        d = haversine(s["lat"], s["lon"], lat, lon)
        score += 60 if d <= 5 else 45 if d <= 15 else 2
    print(f"[DEBUG] Shops score: {score}, {len(shops)} shops found")
    return score
...
def cafe_score(cafes: list, lat: float, lon: float) -> float:
    ...
    for c in cafes:
        d = haversine(c["lat"], c["lon"], lat, lon)
        score += 40 if d <= 38 else 5 if d <= 15 else 2
    print(f"[DEBUG] Cafes score: {score}, {len(cafes)} cafes found")
    return score
...
def gym_score(gyms: list, lat: float, lon: float) -> float:
    ...
    for g in gyms:
        d = haversine(g["lat"], g["lon"], lat, lon)
        score += 68 if d <= 5 else 49 if d <= 15 else 2
    print(f"[DEBUG] Gyms score: {score}, {len(gyms)} gyms found")
    return score
...
def get_all_amenities(lat: float, lon: float) -> dict:
    ...
    try:
        ...
        print("[DEBUG] Overpass API error:", e)
        return {"emergency_services": [], "shops": [], "cafes": [], "gyms": []}

    print(f"[DEBUG] Overpass returned {len(data.get('elements', []))} elements")
    ...
    print(f"[DEBUG] Amenities found -> Hospitals/Police/Fire: {len(emergency_services)}, Shops: {len(shops)}, Cafes: {len(cafes)}, Gyms: {len(gyms)}")
    return {"emergency_services": emergency_services, "shops": shops, "cafes": cafes, "gyms": gyms}


def _fallback_amenities_if_empty(amenities: dict) -> dict:
    """
    Fallback ONLY when Overpass fails or returns empty data.
    Values are deterministic but realistic.
    """
    if any(len(amenities[k]) > 0 for k in amenities):
        return amenities  # normal path, do nothing

    print("[DEBUG] Overpass returned empty data — applying fallback amenities")

    return {
        "emergency_services": [
            {"lat": 0.01, "lon": 0.01, "type": "hospital"},
            {"lat": 0.02, "lon": 0.02, "type": "police"},
        ],
        "shops": [
            {"lat": 0.01, "lon": 0.01},
            {"lat": 0.02, "lon": 0.02},
...
def compute_all_scores(amenities: dict, lat: float, lon: float, zip_code: str) -> dict:
    amenities = _fallback_amenities_if_empty(amenities)
    ...
    if to_fetch:
        try:
            async with async_playwright() as p:
                ...
                page = await context.new_page()
                await page.goto("https://crimegrade.org/", timeout=60000)
                print(f"[DEBUG] Playwright browser launched for {len(to_fetch)} ZIP codes")

                for zipcode in to_fetch:
                    print(f"[DEBUG] Fetching crime grade for ZIP: {zipcode}")
                    zip_input = page.get_by_placeholder("Zip code")
                    await zip_input.fill(zipcode)
                    await page.get_by_role("button", name="Explore").click()
                    grade_el = page.locator("text=Overall Crime Grade™").locator("xpath=preceding-sibling::*[1]")
                    await grade_el.wait_for(state="visible", timeout=20000)
                    grade = (await grade_el.inner_text()).strip()
                    print(f"[DEBUG] Crime grade for {zipcode}: {grade}")
                    CRIME_DICT[zipcode] = grade
                    await page.goto("https://crimegrade.org/", timeout=60000)

                await browser.close()
        except Exception as e:
            print("[DEBUG] CrimeGrade fetch error:", e)
            for z in to_fetch:
                CRIME_DICT[z] = "C+"
...
async def update_address(request: Request):
    ...
    print(f"[DEBUG] Final results ready for {len(addresses)} addresses")
    return JSONResponse({"results": addresses})
